<section id="form-penjualan" aria-labelledby="formulir-heading" class="section-overlap bg-white pt-20 pb-36 md:pb-48 z-40 relative">
                ...
                <!-- Riwayat Servis / Memori Kartu Servis -->
                    <div x-data="{
                        ...
                        <div x-show="displayedServices.length > 0" class="mb-10 pb-8 border-b border-gray-200" x-cloak>
                            <h3 class="text-black text-lg md:text-xl font-bold font-public uppercase mb-4">Riwayat Servis Anda</h3>
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                <template x-for="code in displayedServices" :key="code">
                                    <a :href="'{{ url('/servis/lacak') }}/' + code"
                                        ...
                                        <span class="text-xs text-gray-500 font-bold uppercase tracking-widest flex items-center gap-1 group-hover:text-black font-public">
                    ...
                    <div class="text-center mb-10">
                        <h2 id="formulir-heading" class="text-3xl md:text-4xl font-black font-public uppercase tracking-tighter text-black mb-2">Form Pengajuan Servis</h2>
                        <p class="text-gray-500 font-inter text-sm md:text-base">Silakan isi data dengan lengkap agar teknisi kami dapat memahami kendala perangkat Anda.</p>
                    </div>
                    ...
                    <form wire:submit.prevent="submit" class="flex flex-col gap-6" x-data="mediaUploader()">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                @error('nama') <p class="text-red-600 text-xs mt-1 font-inter">{{ $message }}</p> @enderror
                            </div>
                            <div>
                                ...
                                <input wire:model="whatsapp" id="whatsapp" type="tel" placeholder="Contoh: 08123456789"
                                    class="w-full bg-gray-50 border border-gray-200 rounded-2xl p-4 text-base focus:bg-white focus:outline-none focus:border-black focus:ring-1 focus:ring-black transition-all" />
                                @error('whatsapp') <p class="text-red-600 text-xs mt-1 font-inter">{{ $message }}</p> @enderror
                            </div>
                        </div>

                        <div>
                            <label for="email" class="block text-sm font-bold uppercase tracking-widest mb-2 text-gray-700 font-public">Alamat Email</label>
                            <input wire:model="email" id="email" type="email" placeholder="email@contoh.com"
                                class="w-full bg-gray-50 border border-gray-200 rounded-2xl p-4 text-base focus:bg-white focus:outline-none focus:border-black focus:ring-1 focus:ring-black transition-all" />
                            @error('email') <p class="text-red-600 text-xs mt-1 font-inter">{{ $message }}</p> @enderror
                        </div>

                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <label for="kategori" class="block text-sm font-bold uppercase tracking-widest mb-2 text-gray-700 font-public">Kategori Perangkat</label>
                                ...
                                @error('kategori') <p class="text-red-600 text-xs mt-1 font-inter">{{ $message }}</p> @enderror
                            </div>
                            <div>
                                ...
                                <input wire:model="merek" id="merek" type="text" placeholder="Contoh: LG Smart TV 43 Inch / Kulkas Sharp 2 Pintu"
                                    class="w-full bg-gray-50 border border-gray-200 rounded-2xl p-4 text-base focus:bg-white focus:outline-none focus:border-black focus:ring-1 focus:ring-black transition-all" />
                                @error('merek') <p class="text-red-600 text-xs mt-1 font-inter">{{ $message }}</p> @enderror
                            </div>
                        </div>

                        <div>
                            <label for="deskripsi" class="block text-sm font-bold uppercase tracking-widest mb-2 text-gray-700 font-public">Deskripsi Keluhan</label>
                            <textarea wire:model="deskripsi" id="deskripsi" rows="4" placeholder="Jelaskan masalah yang dialami secara spesifik (Contoh: Kulkas kurang dingin, TV mati total, dll)"
                            ...
                            @error('deskripsi') <p class="text-red-600 text-xs mt-1 font-inter">{{ $message }}</p> @enderror
                        </div>

                        @if($serviceType === 'datang')
                            <div id="alamat-container">
                                <label class="block text-sm font-bold uppercase tracking-widest mb-2 text-gray-700 font-public">Alamat Kunjungan <span class="text-red-500 text-xs ml-1">*Wajib untuk Layanan Teknisi Datang</span></label>
                                ...
                                @error('province_id') <p class="text-red-600 text-xs mt-1 font-inter">{{ $message }}</p> @enderror
                                @error('regency_id') <p class="text-red-600 text-xs mt-1 font-inter">{{ $message }}</p> @enderror
                                @error('district_id') <p class="text-red-600 text-xs mt-1 font-inter">{{ $message }}</p> @enderror
                                @error('village_id') <p class="text-red-600 text-xs mt-1 font-inter">{{ $message }}</p> @enderror
                                @error('address_detail') <p class="text-red-600 text-xs mt-1 font-inter">{{ $message }}</p> @enderror
                            </div>
                        @endif

                        {{-- Upload Foto/Video dengan auto-compress --}}
                        <div>
                            <label class="block text-sm font-bold uppercase tracking-widest mb-2 text-gray-700 font-public">Upload Foto/Video Kendala (Opsional, Max 5 file)</label>
                            <label for="upload-input"
                                ...
                                <p class="text-black font-bold text-base mb-1 font-public">Klik untuk mengunggah foto/video</p>
                                <p class="text-gray-500 text-sm font-inter">Foto max 5MB, Video otomatis dikompres sebelum diunggah</p>
                                <span class="px-5 py-2.5 bg-black text-white text-xs font-bold font-public uppercase tracking-widest rounded-full mt-4 group-hover:bg-brand-yellow group-hover:text-black transition-colors">Pilih File</span>
                                <input x-ref="fileInput" x-on:change="handleFiles($event)" id="upload-input" type="file" multiple class="hidden" />
                            </label>
                            @error('media') <p class="text-red-600 text-xs mt-1 font-inter">{{ $message }}</p> @enderror
                            @error('media.*') <p class="text-red-600 text-xs mt-1 font-inter">{{ $message }}</p> @enderror
                        </div>
                        ...
                        <button type="submit"
                            class="mt-6 w-full bg-black text-white py-5 rounded-full font-public font-black uppercase text-lg tracking-widest hover:bg-gray-800 transition-colors btn-hover flex justify-center items-center gap-3 disabled:opacity-50 disabled:cursor-not-allowed"
    ...
    @if ($submitted)
        ...
        <div wire:key="success-overlay" x-data="{ 
            ...
            x-init="saveToLocal(); initConfetti();"
        ...
        >
            <div class="bg-white rounded-[2.5rem] p-6 sm:p-12 shadow-2xl max-w-lg w-full text-center relative border border-emerald-100 overflow-hidden transform transition-all mt-auto mb-0">
                ...
                <h2 class="text-3xl sm:text-4xl font-black font-public uppercase tracking-tighter text-black mb-2">Pengajuan Berhasil!</h2>
                ...
                <div class="bg-gray-50 border-2 border-emerald-500/20 rounded-2xl p-4 flex items-center justify-between mb-6 shadow-inner relative group">
                    <div class="text-left">
                        <span class="block text-[10px] uppercase font-bold tracking-widest text-emerald-600 font-public mb-0.5">Kode Tiket Servis</span>
                ...
                @php
                    ...
                    $waText = "Kode servis saya di Prokar Elektronik:\n*" . $newServiceCode . "*\n\nLink lacak: " . url('/servis/lacak?code=' . $newServiceCode);
                ...
                video.onloadedmetadata = () => {
                    ...
                    const updateProgress = () => {
                        if (!video.paused && !video.ended) {
                            const prog = Math.min(99, (video.currentTime / duration) * 100);
