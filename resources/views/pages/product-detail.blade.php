...
<style>
  ...
  *, *::before, *::after { box-sizing: border-box; }
  ...
  .prose-desc p          { margin: 0 0 1em; }
  .prose-desc ul         { list-style: disc; padding-left: 1.5em; margin: 0 0 1em; }
  .prose-desc ol         { list-style: decimal; padding-left: 1.5em; margin: 0 0 1em; }
  .prose-desc li         { margin-bottom: .45em; }
  .prose-desc strong, b  { font-weight: 700; }
  .prose-desc br         { display: block; content: ''; margin-top: .6em; }
  .prose-desc h2, .prose-desc h3 { font-weight: 700; font-size: 18px; margin: 1.2em 0 .5em; }
  ...
  .btn-cta:active { opacity: .8; }
  .btn-cart   { background: #fff; border: 2px solid #111; color: #111; }
  ...
  @media (prefers-reduced-motion: no-preference) {
    #mainImage { transition: opacity .15s ease, transform .15s ease; }
  }

  /* ── Push content above fixed bar ─────────── */
  @media (max-width: 1023px) {
    .cta-spacer { height: 80px; }
  }
</style>
@endpush
...
<div class="bg-white border-b border-gray-100">
  <div class="max-w-[1280px] mx-auto px-4 sm:px-6 lg:px-10">
    <nav aria-label="Breadcrumb" class="py-3.5">
      <ol class="flex items-center flex-wrap font-public font-bold text-sm text-[#0A0A0A]/50 uppercase tracking-wider gap-2">
...
<div class="max-w-[1280px] mx-auto px-4 sm:px-6 lg:px-10 py-6 lg:py-10" itemscope itemtype="https://schema.org/Product">
  ...
  <div class="flex flex-col lg:flex-row gap-8 lg:gap-14">
    ...
    <div class="w-full lg:w-[480px] lg:flex-shrink-0 flex flex-col gap-3 sm:gap-4">
      ...
      {{-- Gambar Utama --}}
      <div class="relative w-full aspect-square bg-[#f8fafc] rounded-2xl sm:rounded-3xl overflow-hidden border border-gray-200/80 shadow-xs flex flex-col justify-between group">

        @if ($product->is_promo)
          <span class="absolute top-3.5 left-3.5 z-10 bg-red-600 text-white text-[11px] font-inter font-black px-3 py-1 rounded-full uppercase tracking-wider shadow-sm">PROMO</span>
        @endif

        <div id="mainImageContainer" class="relative flex-1 min-h-0 w-full flex items-center justify-center p-3 sm:p-5 overflow-hidden">
          {{-- Skeleton / Shimmer Placeholder --}}
          <div id="mainSkeleton" class="absolute inset-4 rounded-2xl bg-gradient-to-r from-gray-100 via-gray-200 to-gray-100 animate-pulse flex flex-col items-center justify-center pointer-events-none z-0">
        ...
        <div class="shrink-0 w-full bg-[#008276] text-white py-2 sm:py-2.5 px-4 flex items-center justify-between text-[11px] sm:text-xs font-inter font-semibold z-10">
        ...
        @if ($images->count() > 0)
          @foreach ($images as $index => $image)
            <button
              ...
              @if($image->type === 'video')
                onclick="setMainVideo(this, '{{ $image->url }}')"
              @else
                onclick="setMain(this, '{{ $image->url }}')"
              @endif
              ...
              @if($image->type === 'video')
                <div class="w-full h-full rounded-lg bg-gray-900 flex items-center justify-center relative overflow-hidden group/vid">
                  <div class="w-8 h-8 rounded-full bg-white/20 flex items-center justify-center backdrop-blur-xs group-hover/vid:scale-110 transition-transform">
      ...
      <div class="hidden lg:block mt-6">
        @if ($product->status === 'available')
          <livewire:frontend.add-to-cart-button :product-id="$product->id" wire:key="add-to-cart-desktop-{{ $product->id }}" />
        @elseif ($product->status === 'sold')
          <div class="w-full bg-gray-100 rounded-xl py-4 text-center border border-gray-200">
            <span class="text-gray-400 font-inter font-bold text-sm uppercase tracking-wide">Produk Sudah Terjual</span>
          </div>
        @else
          <div class="w-full bg-gray-100 rounded-xl py-4 text-center border border-gray-200">
            <span class="text-gray-400 font-inter font-bold text-sm uppercase tracking-wide">Tidak Tersedia</span>
          </div>
        @endif
      </div>
    ...
    <div class="w-full lg:flex-1 flex flex-col" itemprop="offers" itemscope itemtype="https://schema.org/Offer">
      ...
      @endphp
      <div class="mb-3">
        <span class="inline-block {{ $badgeClass }} text-white font-public font-bold text-xs md:text-sm px-3 py-1.5 rounded-sm uppercase tracking-wide">
          {{ $product->condition ?? $product->condition_notes ?? 'Kondisi Baik' }}
        </span>
      </div>

      {{-- Nama Produk --}}
      <h1 class="font-inter font-bold text-[26px] sm:text-[30px] lg:text-[34px] leading-tight text-gray-900 mb-4" itemprop="name">
        {{ $product->name }}
      </h1>
      ...
      <div class="mb-5 pb-5 border-b border-gray-100">
        ...
        @endif
        <p class="font-inter font-bold text-[36px] sm:text-[40px] text-gray-900 leading-none" itemprop="price" content="{{ $product->price }}">
      ...
      <div class="mb-6">
        ...
        <div class="spec-row">
          ...
          <span class="spec-value">
            <a href="{{ route('produk.index') }}?kategori={{ $product->category?->slug ?? 'lainnya' }}" class="text-blue-600 hover:underline">
        ...
        <div class="spec-row">
          ...
          <span class="spec-value {{ $product->status === 'available' ? 'text-emerald-700' : 'text-red-600' }}">
      ...
      <article class="related-card" role="listitem">
        <a href="{{ route('produk.show', $related->slug) }}" class="flex flex-col h-full" aria-label="{{ $related->name }}">
          ...
          <div class="p-4 flex flex-col flex-1">
            <span class="text-xs font-inter text-gray-400 uppercase tracking-wide mb-1">{{ $related->category?->name ?? 'Lainnya' }}</span>
