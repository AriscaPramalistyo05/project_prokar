...
<div class="space-y-12">
  ...
  {{-- Section: Role Workflows (SOP Teknisi & Panel Admin) --}}
  <div class="border-t border-slate-200 dark:border-slate-800 pt-8 space-y-6">
    <div class="flex items-center justify-between">
      <div>
        <h2 class="text-lg font-bold text-slate-900 dark:text-white tracking-tight">
          Operasional &amp; Manajemen Sistem
        </h2>
        <p class="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
          Prosedur kerja internal teknisi workshop dan pengaturan kendali administrator.
        </p>
      </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
      ...
      {{-- SOP Teknisi --}}
      <div class="border border-slate-200 dark:border-slate-800 rounded-xl p-5 bg-slate-50/60 dark:bg-slate-900/60 hover:border-slate-300 dark:hover:border-slate-700 transition-all flex flex-col justify-between">
        <div>
          <div class="flex items-center justify-between mb-3">
            <span class="text-[11px] font-mono font-medium px-2 py-0.5 rounded bg-sky-50 text-sky-700 border border-sky-200 dark:bg-sky-950/30 dark:text-sky-400 dark:border-sky-900/50">Wewenang: Teknisi</span>
            <span class="text-xs text-slate-400 font-mono">Panel Teknisi</span>
          </div>

          <h3 class="text-base font-bold text-slate-900 dark:text-white">
            <a href="{{ url('/docs/teknisi') }}" class="hover:text-sky-600 dark:hover:text-sky-400 transition-colors">
              SOP Teknisi
            </a>
          </h3>

          <p class="mt-1 text-xs text-slate-500 dark:text-slate-400 leading-relaxed">
            Standar penanganan fisik perangkat, pengetesan tegangan komponen, penginputan estimasi suku cadang, dan uji kelayakan QC.
          </p>

          <ul class="mt-3 space-y-1.5 text-xs">
            <li>
              <a href="{{ url('/docs/sop-penerimaan-dan-diagnosa-teknisi') }}" class="text-slate-600 dark:text-slate-400 hover:text-sky-600 dark:hover:text-sky-400 flex items-center gap-1.5">
                <span class="text-slate-400">&bull;</span>
                <span>Penerimaan &amp; Diagnosa</span>
              </a>
            </li>
            <li>
              <a href="{{ url('/docs/sop-input-estimasi-dan-sparepart') }}" class="text-slate-600 dark:text-slate-400 hover:text-sky-600 dark:hover:text-sky-400 flex items-center gap-1.5">
                <span class="text-slate-400">&bull;</span>
                <span>Estimasi &amp; Sparepart</span>
              </a>
            </li>
            <li>
              <a href="{{ url('/docs/sop-quality-control-dan-serah-terima') }}" class="text-slate-600 dark:text-slate-400 hover:text-sky-600 dark:hover:text-sky-400 flex items-center gap-1.5">
                <span class="text-slate-400">&bull;</span>
                <span>QC &amp; Aktivasi Garansi</span>
              </a>
            </li>
          </ul>
        </div>

        <div class="mt-4 pt-3 border-t border-slate-200/70 dark:border-slate-800">
          <a href="{{ url('/docs/teknisi') }}" class="text-xs font-semibold text-sky-600 dark:text-sky-400 hover:underline flex items-center justify-between">
            <span>Buka Seluruh SOP Teknisi</span>
            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M13.5 4.5 21 12m0 0-7.5 7.5M21 12H3"/></svg>
          </a>
        </div>
      </div>
      ...
      <div class="border border-slate-200 dark:border-slate-800 rounded-xl p-5 bg-slate-50/60 dark:bg-slate-900/60 hover:border-slate-300 dark:hover:border-slate-700 transition-all flex flex-col justify-between">
        <div>
          <div class="flex items-center justify-between mb-3">
            <span class="text-[11px] font-mono font-medium px-2 py-0.5 rounded bg-amber-50 text-amber-700 border border-amber-200 dark:bg-amber-950/30 dark:text-amber-400 dark:border-amber-900/50">Wewenang: Super Admin</span>
            <span class="text-xs text-slate-400 font-mono">Pusat Kendali</span>
          </div>

          <h3 class="text-base font-bold text-slate-900 dark:text-white">
            <a href="{{ url('/docs/admin') }}" class="hover:text-amber-600 dark:hover:text-amber-400 transition-colors">
              Super Admin
            </a>
          </h3>

          <p class="mt-1 text-xs text-slate-500 dark:text-slate-400 leading-relaxed">
            Kontrol penuh data pesanan, antrean tiket servis, approval pembelian unit bekas, manajemen hak akses staf, dan konfigurasi toko.
          </p>

          <ul class="mt-3 space-y-1.5 text-xs">
            <li>
              <a href="{{ url('/docs/admin-manajemen-produk-dan-pesanan') }}" class="text-slate-600 dark:text-slate-400 hover:text-amber-600 dark:hover:text-amber-400 flex items-center gap-1.5">
                <span class="text-slate-400">&bull;</span>
                <span>Katalog &amp; Transaksi</span>
              </a>
            </li>
            <li>
              <a href="{{ url('/docs/admin-penugasan-dan-pengaturan-toko') }}" class="text-slate-600 dark:text-slate-400 hover:text-amber-600 dark:hover:text-amber-400 flex items-center gap-1.5">
                <span class="text-slate-400">&bull;</span>
                <span>Penugasan &amp; Pengaturan</span>
              </a>
            </li>
          </ul>
        </div>
        ...
        <div class="mt-4 pt-3 border-t border-slate-200/70 dark:border-slate-800">
          <a href="{{ url('/docs/admin') }}" class="text-xs font-semibold text-amber-600 dark:text-amber-400 hover:underline flex items-center justify-between">
            <span>Buka Seluruh Panduan Admin</span>
            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M13.5 4.5 21 12m0 0-7.5 7.5M21 12H3"/></svg>
          </a>
        </div>
      </div>
