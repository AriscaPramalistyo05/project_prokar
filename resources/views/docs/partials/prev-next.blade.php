...
<div class="flex flex-col sm:flex-row gap-3 mt-12 pt-6 border-t border-slate-200 dark:border-slate-800">
  @if($previous)
    <a href="{{ route('docs.show', [$category->slug, $previous->slug]) }}" class="docs-nav-card flex-1">
  ...
  @if($next)
    <a href="{{ route('docs.show', [$category->slug, $next->slug]) }}" class="docs-nav-card flex-1 text-right">
